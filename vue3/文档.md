https://composition-api.vuejs.org/zh/api.html#shallowreadonly
## `setup`
...
`⚠️：何时需要写,value., setup（props, ctx）`

**在setUp中使用 ref 创建的变量时，需要使用.value**

```js
setup() {
	const count =  ref(0)
  
  // 在 watch 和 computed 中使用 count 变量的时候需要加上.value
}
```
...
`⚠️这个问题就是 vue3会存在的一个问题，对象属性的响应式丢失,所以不能使用结构，结构会导致响应式丢失的问题`
...
⚠️注意当嵌套在 reactive `Object` 中时，ref 才会解套。从 `Array` 或者 `Map` 等原生集合类中访问 ref 时，不会自动解套：
...
`⚠️toRef 可以用来解决reactive 创建对象属性时会遇到的响应式丢失的问题`

`toRef` 可以用来为一个 reactive 对象的`属性创建一个 ref`。这个 ref 可以被传递并且能够保持响应性。
...
### [#](https://composition-api.vuejs.org/zh/api.html#isproxy)`isProxy`

检查一个对象是否是由 `reactive` 或者 `readonly` 方法创建的代理。

### [#](https://composition-api.vuejs.org/zh/api.html#isreactive)`isReactive`

检查一个对象是否是由 `reactive` 创建的响应式代理。

如果这个代理是由 `readonly` 创建的，但是又被 `reactive` 创建的另一个代理包裹了一层，那么同样也会返回 `true`。

### [#](https://composition-api.vuejs.org/zh/api.html#isreadonly)`isReadonly`
...
### [#](https://composition-api.vuejs.org/zh/api.html#shallowreadonly)`shallowReadonly`
